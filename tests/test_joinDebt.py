...
def test_joinDebt(fn_isolation, contracts, users, delegates, admin):
    ...
    reward_rate1 = reward_incr1 * 1e18 / iotx_clear.getTotalDebts()
    account = users[1]
    tx = iotx_clear.joinDebt(account, amount, {"from": iotx_staking})
    assert "DebtQueued" in tx.events
    assert iotx_clear.getTotalDebts() == amount*2
    ...
    assert iotx_clear.getAccountedBalance() == reward_incr1
    assert iotx_clear.getRewardRate() == reward_rate1

    reward_incr2 = 200
    delegates[0].transfer(iotx_clear, reward_incr2)
    account = users[0]
    reward_rate2 = reward_rate1 + (reward_incr2 * 1e18 / iotx_clear.getTotalDebts())
    ...
    tx = iotx_clear.joinDebt(account, amount, {"from": iotx_staking})
    assert "DebtQueued" in tx.events
    assert iotx_clear.getTotalDebts() == amount*3
    ...
    assert iotx_clear.getAccountedBalance() == reward_incr1 + reward_incr2
    assert iotx_clear.getRewardRate() == reward_rate2

    reward_incr3 = 300
    delegates[0].transfer(iotx_clear, reward_incr3)
    account = users[1]
    reward_rate3 = reward_rate2 + (reward_incr3 * 1e18 / iotx_clear.getTotalDebts())
    ...
    tx = iotx_clear.joinDebt(account, amount, {"from": iotx_staking})
    assert "DebtQueued" in tx.events
    assert iotx_clear.getTotalDebts() == amount*4
    ...
    assert iotx_clear.getAccountedBalance() == reward_incr1 + reward_incr2 + reward_incr3
    assert iotx_clear.getRewardRate() == reward_rate3
