def test_getRedeemedTokenIdsSlice(fn_isolation, contracts, users, deadline):
    ...
    token_ids = iotx_staking.getRedeemedTokenIdsSlice(0, 0)
    assert len(token_ids) == 0
    token_ids = iotx_staking.getRedeemedTokenIdsSlice(1, 0)
    assert len(token_ids) == 0
    token_ids = iotx_staking.getRedeemedTokenIdsSlice(0, iotx_staking.getRedeemedTokenCount() + 1)
    assert len(token_ids) == 0
