def test_getRedeemedTokenIdSlice(fn_isolation, contracts, users, deadline):
    ...
    token_ids = iotx_staking.getRedeemedTokenIdSlice(0, 0)
    assert len(token_ids) == 0
    token_ids = iotx_staking.getRedeemedTokenIdSlice(1, 0)
    assert len(token_ids) == 0
    token_ids = iotx_staking.getRedeemedTokenIdSlice(0, iotx_staking.getRedeemedTokenCount() + 1)
    assert len(token_ids) == 0
